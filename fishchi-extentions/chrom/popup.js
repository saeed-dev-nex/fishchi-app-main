...
(function () {
  ...
  const elements = {
    ...
    doiSection: document.getElementById("doiSection"),
    ...
    // DOI Section elements
    doiUserInfo: document.getElementById("doiUserInfo"),
    doiUserName: document.getElementById("doiUserName"),
    doiUserEmail: document.getElementById("doiUserEmail"),
    doiLogoutBtn: document.getElementById("doiLogoutBtn"),
    doiInput: document.getElementById("doiInput"),
    doiProjectSelect: document.getElementById("doiProjectSelect"),
    importDoiBtn: document.getElementById("importDoiBtn"),
    extractFromPageBtn: document.getElementById("extractFromPageBtn"),
    importDoiFromPageBtn: document.getElementById("importDoiFromPageBtn"),
  ...
  async function checkAuthStatus() {
    try {
      ...
      if (response.authenticated) {
        ...
        // Check if we should show DOI section or Extract section based on current page
  ...
  function setupEventListeners() {
    ...
    // DOI event listeners
    if (elements.doiLogoutBtn) {
      elements.doiLogoutBtn.addEventListener("click", handleLogout);
    }
    if (elements.importDoiBtn) {
      elements.importDoiBtn.addEventListener("click", handleDoiImport);
    }
    if (elements.extractFromPageBtn) {
      elements.extractFromPageBtn.addEventListener("click", () => {
        showExtractSection();
        refreshSourceInfo();
      });
    }
    if (elements.importDoiFromPageBtn) {
      elements.importDoiFromPageBtn.addEventListener("click", () => {
        showDoiSection();
      });
    }
    if (elements.doiInput) {
      elements.doiInput.addEventListener("input", validateDoi);
      // Add example DOI click handlers
      const exampleDois = document.querySelectorAll(".example-doi");
      exampleDois.forEach((example) => {
        example.addEventListener("click", () => {
          const doi = example.getAttribute("data-doi");
          elements.doiInput.value = doi;
          validateDoi();
        });
      });
    }
  ...
  async function handleExtract() {
    if (!currentSourceInfo) {
      showError("اطلاعات منبع یافت نشد. لطفاً ابتدا صفحه را بروزرسانی کنید");
      return;
    }
  ...
  async function handleDoiImport() {
    const doi = elements.doiInput.value.trim();

    if (!doi) {
      showError("لطفاً DOI را وارد کنید");
      return;
    }

    if (!isValidDoi(doi)) {
      showError("DOI وارد شده معتبر نیست");
      return;
    }

    const projectId = elements.doiProjectSelect.value || null;
    ...
    try {
      ...
      if (response.success) {
        ...
        showError(response.message || "خطا در وارد کردن منبع از DOI");
      }
    } catch (error) {
  ...
  // Update DOI user info
  function updateDoiUserInfo() {
    if (userData) {
      if (elements.doiUserName) {
        elements.doiUserName.textContent =
          userData.name || userData.firstName || "کاربر";
      }
      if (elements.doiUserEmail) {
        elements.doiUserEmail.textContent = userData.email || "ایمیل نامشخص";
      }
    } else {
      if (elements.doiUserName) {
        elements.doiUserName.textContent = "کاربر";
      }
      if (elements.doiUserEmail) {
        elements.doiUserEmail.textContent = "ایمیل نامشخص";
      }
    }
  }

  // Update DOI project select
  function updateDoiProjectSelect() {
    if (!elements.doiProjectSelect) return;

    elements.doiProjectSelect.innerHTML =
    ...
    projects.forEach((project) => {
      ...
      option.value = project._id;
      option.textContent = project.title;
      elements.doiProjectSelect.appendChild(option);
    });
  }

  // Set DOI loading state
  function setDoiLoading(isLoading) {
    if (elements.importDoiBtn) {
      elements.importDoiBtn.disabled = isLoading;
      const btnText = elements.importDoiBtn.querySelector(".btn-text");
      const btnLoader = elements.importDoiBtn.querySelector(".btn-loader");
    ...
    if (isLoading) {
      updateStatus("loading", "در حال وارد کردن DOI...");
    }
  }
  ...
  function showDoiSection() {
    hideAllSections();
    elements.doiSection.style.display = "block";
    updateDoiUserInfo();
    updateDoiProjectSelect();
    currentState = "doi";
  }
  ...
  async function determineInitialView() {
    try {
      ...
      // Check if current tab is a supported extraction site
      const extractionSites = ["sid.ir", "civilica.com", "noormags.ir"];
      const isExtractionSite = extractionSites.some((site) =>
        tab.url.includes(site),
      );

      // Check if current tab is a DOI-supported site
      const doiSites = [
        "sciencedirect.com",
        "springer.com",
        "ieee.org",
      ...
      const isDoiSite = doiSites.some((site) => tab.url.includes(site));
      ...
      showDoiSection(); // Default to DOI section
    }
  }
  ...
  function setLoading(isLoading, text = "") {
    if (currentState === "login") {
      ...
      const loginBtnText = elements.loginBtn.querySelector(".btn-text");
      const loginBtnLoader = elements.loginBtn.querySelector(".btn-loader");
      if (loginBtnText && loginBtnLoader) {
        loginBtnText.style.display = isLoading ? "none" : "inline";
        loginBtnLoader.style.display = isLoading ? "block" : "none";
      }
    ...
    } else if (currentState === "doi") {
      setDoiLoading(isLoading);
    }
  ...
  // Add DOI debugging function
  async function debugDoiImport() {
    ...
    try {
      ...
      showDebugResult("تست وارد کردن DOI", response, response.success);
    } catch (error) {
      showDebugResult("تست وارد کردن DOI", { error: error.message }, false);
    }
  }
